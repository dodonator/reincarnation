...
# Default shapes
	...
	def draw_pentagram(self, l, e, name='default'):
		for i in range(e):
			self.rt((360/e)*2,name)
			self.fd(l,name)
			self.rt((360/e)*2,name)
			self.fd(l,name)
			self.lt(360/e,name)
...
# The reincarnation of the dodo has begun! Fear the power of the primitive ratites!
    ...
    def generateSimpleArmy(self,length):
		soldierNames = []
		for i in range(length):
			tmpName = 'dodo#' + str(i)
			self.reincarnationOfTheDodos(tmpName)
			soldierNames.append(tmpName)
		if length + 1 == self.get_length_of_Army():
			return soldierNames			
		else:
			return soldierNames
